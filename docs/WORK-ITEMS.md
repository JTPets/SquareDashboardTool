...
**0a: Dead `last_cost_cents` columns FIXED** — Dropped `last_cost_cents` and `last_cost_date` from `variations` table. Migration `005_drop_last_cost_cents.sql`. All read references (loyalty-reports.js) replaced with LATERAL JOINs to `variation_vendors` (cheapest vendor pattern). Removed from `variation-service.js` allowlist, `schema.sql`, `schema-manager.js`. 3 tests updated.

**0b: PO receive vendor cost update FIXED** — When receiving a PO, if any line item's cost differs from the current `variation_vendors.unit_cost_money`, the vendor link is upserted with the new cost. Runs inside the existing receive transaction for atomicity. Uses ON CONFLICT upsert pattern. 2 tests added.

**0c: Square default vendor flag INVESTIGATED** — Square Catalog API `vendor_information` array does not expose a "default" or "primary" vendor flag. No `is_default`, `is_primary`, or priority field exists. Documented in TECHNICAL_DEBT.md.

**BACKLOG-79 FIXED** — Cron job schedule audit. Rescheduled 7 batch jobs into 2–6 AM ET window: Cycle count (1AM→3AM), Webhook retry (every min→every 5 min), Expiry discount (6AM→5AM), Seniors discount (12:30AM→2:30AM), Trial expiry (7AM→5AM), Catalog health (2AM→4AM), Email heartbeat (8AM→6AM). `.env.example` defaults updated.

**BACKLOG-88 FIXED** — Tax selection on bulk item creation. Added optional `tax_ids` array parameter to `POST /api/vendor-catalog/create-items`. Added `GET /api/vendor-catalog/merchant-taxes` endpoint to fetch Square tax objects. Backward compatible: omitting `tax_ids` auto-applies all active taxes. 3 tests added.

**BACKLOG-90 FIXED** — Vendor catalog import now proposes vendor links instead of auto-creating. Import returns `suggestedLinks` array when UPC matches exist but variation isn't linked to the importing vendor. Added `POST /api/vendor-catalog/confirm-links` endpoint to create confirmed links. Frontend section with checkboxes and "Link Selected" button added. 3 tests added.

**BACKLOG-91 FIXED** — PO minimum threshold changed from hard-block to soft-block. Manual PO creation with `force: true` proceeds below vendor minimum with a warning. Without `force` parameter, behavior unchanged (hard block for automation). 2 tests added.

**BACKLOG-94 FIXED** — Expiry quantity tracking. Migration `006_expiry_quantity_tracking.sql` adds `expiring_quantity` and `units_sold_at_discount` columns to `variation_discount_status`. `trackExpiryDiscountSale()` increments sold count on each order and flags for manual review when threshold reached. Hooked into order webhook handler (non-blocking). `PATCH /api/expiry-discounts/variations/:variationId/quantity` endpoint added. 9 tests added.
...
| BACKLOG-39 | Vendor bill-back tracking + promo engine — three connected pieces: (1) **Promo engine**: custom coupon/bundle creator outside Square's pricing rules. Square's timed pricing rules show items as "on sale" on the Square website even when the sale is dormant/inactive — must bypass this by managing promos internally. Create named promo bundles (group of items), set discount ($ or %), set active date range (start → end). (2) **Discount application**: apply discounts at order time via order-level or line-item discounts, NOT via Square catalog pricing rules — keeps catalog clean and website accurate. (3) **Bill-back reporting**: tie each promo to a vendor agreement, aggregate actual sales during promo period per vendor for claim submission. Two bill-back types: *promo bill-backs* (vendor-funded promos with date range, e.g., "Smack March Promo — 15% off X items, Mar 1-15, Smack covers discount") and *seniors day bill-backs* (recurring — vendors like Smack agree to cover the 10% seniors day discount on their specific items/brands). Revenue recovery feature. | New tables (`vendor_promos`, `vendor_promo_items`, `vendor_billbacks`, `vendor_billback_claims`) + routes + services + UI | L | 2026-02-01 |
...
| BACKLOG-81 | Margin erosion tracking — three sources of margin impact need unified tracking: (1) Cost/price changes from vendor imports and Square updates (original scope). (2) Loyalty redemptions: every free item is a margin event. Track retail given away, wholesale cost absorbed, vendor credit received, net margin impact per redemption. The vendor receipt now reports all 3 values (retail, lowest paid, wholesale). (3) Expiry discounts: items sold at 25%/50% off are margin hits. Connect to existing variation_discount_status tracking. Goal: single dashboard showing total margin erosion by source (cost changes, loyalty giveaways, expiry markdowns) per period. | New service, `services/reports/`, `services/loyalty-admin/` | L | 2026-03-23 |
...
| BACKLOG-110 | Webhook-triggered PO receive prompt — when `inventory.count.updated` webhook fires with a quantity increase for a variation on an open PO (DRAFT or SUBMITTED), flag the PO with a notification: "Inventory received — review PO-XXXX". Show on dashboard and reorder page. Do not auto-complete — let merchant confirm (quantity increases happen for many reasons: cycle counts, returns, transfers). Links to BACKLOG-99 (PO inventory push) and BACKLOG-108 (stale draft visibility). Implementation: add check in `services/webhook-handlers/inventory-handler.js`, cross-reference `purchase_order_items` for open POs, create notification record. | `services/webhook-handlers/inventory-handler.js`, `routes/purchase-orders.js` | M | 2026-03-31 |
