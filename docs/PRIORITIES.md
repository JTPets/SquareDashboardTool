...
| ~~SEC-14~~ | ~~12 delivery/driver API endpoints lack per-endpoint rate limiting beyond global 100/15min cap. Includes public 10MB file upload (`POST /api/driver/:token/orders/:orderId/pod`)~~ — **RESOLVED 2026-03-06**: Added `deliveryRateLimit` to 7 delivery.js mutation endpoints + 5 driver-api.js endpoints. Added `deliveryStrictRateLimit` to `POST /backfill-customers` and public POD upload. 31 tests in `delivery-rate-limiting.test.js`. | DEAD-CODE-REPORT-2026-03-05 | M |
